...
- **URL**: `/users/register`
- **是否需要 Token**: 否
- **正常测试**: 用户名 `xiaoling`，密码 `123456`，返回 200 和用户信息
- **异常测试**: 再次使用 `xiaoling` 注册，返回 400，提示 `Username already registered`
...
- **URL**: `/tags/`
- **是否需要 Token**: 是
- **正常测试**: 传入 `name`，返回创建的标签
- **异常测试**: 创建重复名称的标签，返回 400
...
- **是否需要 Token**: 是
- **正常测试**: 返回 `total`、`completed`、`completion_rate`、`application_count`、`tag_count`
- **异常测试**: 不携带 Token，返回 401
